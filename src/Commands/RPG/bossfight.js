...
>>>>>>> [CHANGES] Commit.
		...
		const assets = {
			...
			enemy: await Canvas.loadImage('src/Assets/boss.png'),
...
>>>>>>> [CHANGES] Commit.
		...
		async function checkForWin() {
			if(gameData[1].HP <= 0 || gameData[0].HP <= 0) {
				...
				if(wonByWho === 'WizWiz') {
					const e = await client.db.addAchievement('ACH8', interaction, client);
					data.Achievements = e.Achievements;
					client.db.forceUpdate({ UserId: interaction.member.id }, data, require('../../Schemas/Users'));

					client.createMessage(interaction.channel.id, 'Thanks for playing! (Code: FaceDev, Emojis & Assets: Jerson.EXE)');
				}

				gameEnded = true;
				string += `[END] This game has ended! The winner is ${gameData[1].HP <= 0 ? 'WizWiz' : gameData[1].NAME}.`;
