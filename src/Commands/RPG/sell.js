...
module.exports = class Command extends InteractionBase {
	...
	async run(interaction) {
		const data = await Schema.findOneAndUpdate({ UserId: interaction.member.id }, {}, { new: true, upsert: true, setDefaultsOnInsert: true }).select('Statistics Coins Achievements Backpack').lean();
