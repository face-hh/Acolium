...
class Webhook {
	...
	async login(path, port) {
		if (!path) return console.fatal('Missing path on function: on | Index: 0');
		if (typeof (path) !== 'string') return console.fatal('path typeof is not string on function: on | Index: 0');
		if (!path.startsWith('/')) return console.fatal('Invalid path on function: on | Index: 0');
		if (!port) return console.fatal('Missing port on function: on | Index: 1');
		if (isNaN(port)) return console.fatal('Invalid port on function: on | Index: 1');
